        ...
        $('.steps-validation').steps({
            ...
            onStepChanging: function (event, currentIndex, newIndex) {
                ...
                if (currentIndex > newIndex) {
                    console.log('back click' + newIndex);
                    mobitelsum = 0;
                    airtelsum =0;
                    dialogsum=0;
                    othersum=0;
                ...
                if (currentIndex < newIndex) {
                    ...
                    var charLength = $('#campaignName').val().length;
                    if(charLength >= 3){
                        console.log('ss3')
                    }else {
                        $('#campaignNamespan').text('Please Enter Campaign Name');
                        console.log('ss50')
                        return false;
                ...
                creditbalance = $('#credit').html();
                $('#reqcredit').html(total);
                $('#aftercredit').html(parseInt(creditbalance) - total);
            ...
            onFinished: function (event, currentIndex) {
                ...
                // $('#frmSendMessage').unbind().submit();

                var dataset = new FormData();
                dataset.append('campaignName', $("[name='campaignName']").val());
                dataset.append('promo', $("[name='promo']").val());
                dataset.append('messageType', $("[name='messageType']").val());
                dataset.append('messageText', $("[name='messageText']").val());
                dataset.append('btnText', $("[name='btnText']").val());
                dataset.append('senderId', $("#senderId").val());
                dataset.append('btnUrl', $("[name='btnUrl']").val());
                dataset.append('fallbackMessage', $("[name='fallbackMessage']").val());
                dataset.append('imgTag', $('#imgtagform').val());
                dataset.append('loggedUserId', $('#loggedUser').val());
                dataset.append('action', 'save');
                dataset.append('groupidlist',grouplist);
                ...
                swal({
                    ...
                    onOpen: () => {
                        swal.showLoading();
                        $('.swal2-actions').html('<div class="spinner-box"><div class="circle-border" style=" background: linear-gradient(0deg, rgba(111,75,151,0.1) 33%, rgb(111,75,151) 100%);"><div class="circle-core"></div> </div> </div>');
                        ///
                        $.ajax(path+'ViberMessageController', {
                            method: "POST",
                            data: dataset,
                            processData: false,
                            contentType: false,
